...
class Output:
    ...
    @classmethod
    def build(cls, skills, raw_output: dict, input_type: type = str) -> "Output":
        ...
        if generator < 0:
            return _build_internal(0, skills, input_type)
        else:
    ...
    def attach(self, skill: Skill, data: Union['Output', List[Label]]):
        """
        Add data generated by `Skill` to the output. Data can be a nested `Output` instance or a list of `Label`s.
    ...
    def merge(self, output: "Output") -> "Output":
        """
        Merge data from the given `Output` instance into this one.
